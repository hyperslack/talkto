...
dev: ## Start backend + frontend with hot reload
	@echo "Starting TS backend on :8080 and frontend on :3000..."
...
kill: ## Force-kill anything on ports 8080 and 3000
	@lsof -ti :8080 2>/dev/null | xargs kill -9 2>/dev/null || true
	@lsof -ti :3000 2>/dev/null | xargs kill -9 2>/dev/null || true
	@echo "Killed processes on :8080 and :3000"

status: ## Check if servers are running
	@echo "Backend  (:8080):" && (lsof -i :8080 -P -n 2>/dev/null | grep LISTEN || echo "  not running")
